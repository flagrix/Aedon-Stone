...
public class TableauRcords : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            RecordDuo[i] = PlayerPrefs.GetInt("RecordDuo_" + i, -1); // -1 si aucune valeur enregistr�e
        }
    }
    ...
    public void AffichageScores()
    {
        ...
        Histoire.interactable = false; // Emp�cher les clics
    ...
    public void Croix_Rouge()
    {
        ...
        ColorBlock colors = Histoire.colors;
        colors.normalColor = Color.white; // R�tablir la couleur d'origine
        Histoire.colors = colors;
        ColorBlock colors_rec = Record.colors;
